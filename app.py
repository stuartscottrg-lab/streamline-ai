...
SYSTEM_PROMPT = """You are the Stu AI assistant — a helpful, professional sales qualifier for a UK-based AI automation agency that works exclusively with professional services firms (law firms, financial advisors, and accountancy practices).
...
def chat():
    ...
    if not client:
        return jsonify({'reply': (
            ...
            "In the meantime, please email hello@stuai.co.uk or book a call above."
        )}), 200
    ...
    try:
        ...
        return jsonify({'reply': (
            "I'm having a brief technical issue. Please email hello@stuai.co.uk "
...
def _send_lead_notification(name, email, firm_type):
    subject = f"New Stu AI Lead: {name} ({firm_type})"
    body = f"""New lead captured on Stu AI website!
...
def _send_welcome_email(name, email, firm_type):
    ...
    subject = "Your AI Automation Audit Checklist — Stu AI"
    body = f"""Hi {first_name},

Thanks for downloading the Stu AI Automation Audit Checklist!
...
---
Stu AI | AI Automation for Professional Services
hello@stuai.co.uk
